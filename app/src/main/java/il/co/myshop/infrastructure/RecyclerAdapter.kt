...
abstract class RecyclerAdapter<VH : RecyclerAdapter.ViewHolder<T>, T : RecyclerAdapter.Identity<*>>(
    ...
    var recyclerView: RecyclerView? by weakRef(null)
    protected val viewHolderClasses: MutableMap<Int, Class<ViewHolder<*>>> = mutableMapOf()
    protected open val adapterClasses: List<Class<*>> = emptyList()
    ...
    @Suppress("UNCHECKED_CAST")
    protected open fun createViewHolderInstance(itemView: View): VH {
        var vh: VH?
        if (adapterClasses.isNotEmpty())
            adapterClasses.forEach { adapterClass ->
                vh = catch {
                    viewHolderClass?.getDeclaredConstructor(View::class.java, adapterClass)
                        ?.newInstance(itemView, this)
                } as? VH
                if (vh != null) return vh as VH
            }
        else {
            return catch {
                viewHolderClass?.getDeclaredConstructor(javaClass, View::class.java)
                    ?.newInstance(this, itemView)
            } as VH
        }
        throw Exception()
    }
    ...
    open class ViewHolder<T : Identity<*>>(
        itemView: View
