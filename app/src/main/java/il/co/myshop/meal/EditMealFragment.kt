...
import il.co.lapita.cart.CartAdapter
import il.co.lapita.cart.CartFragment
import il.co.lapita.data.Cart
import il.co.lapita.data.Cart.updateLiveData
import il.co.lapita.data.Meal
import il.co.lapita.data.MealProduct
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        meal?.cartItem?.product?.product?.levels?.forEachIndexed { levelIndex, level ->
            updateList(
                ...
                MealFragment.open(level = levelIndex, Meal(meal?.cartItem?.product ?: return@updateList)){ shopProduct,amount, baseToppings,toppings,options ->
                    ...
                    meal?.cartItem?.meals?.first()?.second?.apply {
                        ...
                        add(levelPosition.plus(itemPosInLevel), MealProduct(shopProduct, amount, levelIndex, level.toppingsFree,  level.toppingsAddPaid == 1, level.optionsPaid == 0, baseToppings, toppings, options))
                        Cart.updateLiveData()
                        menuBar?.toggleCartButton()
                        Cart.save()

                        if (shopProduct.product.productType == ProductType.PIZZA || shopProduct.isToppings || shopProduct.isBaseToppings)
                            main {
                                delay(100)
