...
import il.co.lapita.data.Cart.save
import il.co.lapita.data.Cart.updateLiveData
...
class CartAdapter(
    ...
    inner class ViewHolder(itemView: View) :
        ...
        private fun default() {
            ...
            startEditButton?.onClick {
                when {
                    ...
                    item?.cartItem?.product?.isToppings == true || item?.cartItem?.product?.isBaseToppings == true -> {
                        ToppingsFragment.open(
                            item?.cartItem?.product?.product?.productType == ProductType.PIZZA,
                            item?.cartItem?.product ?: return@onClick,
                            item?.cartItem
                        ) { toppings, options, baseToppings, productId ->
                            toppings?.let {
                                updateCart(productId ?: this.productId, null, it, options, baseToppings)
                                setMode(ItemMode.Default)
                            } ?: cart.remove(productId ?: this.productId)
        ...
        private fun updateCart(
            ...
            baseTopping: MutableList<MutableList<Pair<Int, List<Int>>>>? = null
            ): Cart.Item? {
            ...
            return cart.set(
                ...
                baseToppings = baseTopping ?: item?.cartItem?.baseToppings
            ).also {
