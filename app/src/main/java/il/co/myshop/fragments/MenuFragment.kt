package il.co.lapita.fragments
...
import il.co.lapita.utilities.openWhatsapp
...
class MenuFragment : AbstractFragment() {
    ...
    private val whatsapp: ImageView? get() = menu_whatsapp
    private val facebook: ImageView? get() = menu_facebook
    ...
    private fun initButtons() {
        ...
        whatsapp?.isVisible = Database.shop?.phoneWhatsapp != null
        whatsapp?.onClick {
            Database.shop?.phoneWhatsapp?.let { it1 -> openWhatsapp(it1) }
        }
        facebook?.isVisible = Database.shop?.linkFacebook != null
        facebook?.onClick {
            openWebBrowser(Database.shop?.linkFacebook ?: return@onClick)
        }
    }
