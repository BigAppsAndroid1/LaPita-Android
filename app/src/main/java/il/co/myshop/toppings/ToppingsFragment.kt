...
class ToppingsFragment : BaseFragment() {
    ...
    companion object : Comp() {
        ...
        fun open(
            ...
            level: Level? = null,
            onFinish: (MutableList<MutableList<Pair<Int, List<Int>>>>?, MutableList<MutableList<Pair<Int, List<Int>>>>?, MutableList<MutableList<Pair<Int, List<Int>>>>?, productId: Int?) -> Unit,
        ) {
            open(
                KEY_PIZZA to pizza,
    ...
    private val state by lazy {
        ToppingsState(
            count = cartItem?.toppings?.count()?.minus(1) ?: 0,
            selected = cartItem?.shopToppings?.toMutableList() ?: mutableListOf(),
            options = cartItem?.shopOptions?.toMutableList() ?: mutableListOf(),
            baseToppings = cartItem?.shopBaseToppings?.toMutableList()
                ?: mutableListOf(),
    ...
    var onToppingsSave: ((List<List<Pair<Int, List<Int>>>>?, List<List<Pair<Int, List<Int>>>>?, List<List<Pair<Int, List<Int>>>>?, productId: Int?) -> Unit)? = null;private set
    ...
    override fun onArguments(arguments: Map<String, Any?>) {
        ...
        (arguments[KEY_ON_FINISH] as? (List<List<Pair<Int, List<Int>>>>?, List<List<Pair<Int, List<Int>>>>?, List<List<Pair<Int, List<Int>>>>?, productId: Int?) -> Unit)?.let {
            onToppingsSave = it
        }
    ...
    fun initPager() {
        viewPager?.adapter = product?.let { ToppingsPizzaPagerAdapter(this, it, cartItem, if (level == null) state else ToppingsState(0, mutableListOf(), mutableListOf(), mutableListOf(), mutableMapOf(), mutableListOf()), level) }
