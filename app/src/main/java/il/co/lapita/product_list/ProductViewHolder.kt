...
import il.co.lapita.infrastructure.*
import il.co.lapita.meal.AddOrEditMealDialog
import il.co.lapita.meal.MealFragment
...
import il.co.lapita.widgets.AmountWidget
import il.co.lapita.widgets.UnitTypeWidget
import il.co.lapita.widgets.UnitTypeWidget2
import il.co.lapita.widgets.UnitTypeWidget3
...
class ProductViewHolder(
    ...
    companion object {
        ...
        fun openMeal(product: ShopProduct?) {
            product?.let {
                val meal = if (cart.has(product.id))
                    Meal(it, cart[product.id]?.meals ?: mutableListOf())
                else
                    Meal(it)
    ...
    override fun bind(
        ...
        if (Shop.listType == ListType.Horizontal && foregroundFragment !is SearchFragment) itemView.updateMargins(
            left = 4.dpToPx,
            right = 4.dpToPx
        )
    ...
    private fun displayDetails(product: ShopProduct) {
        ...
        if (Shop.listType != ListType.LinearBig && foregroundFragment !is SearchFragment)
    ...
    private fun toggleInfoButton() {
        product?.product?.description?.takeIf { it.isNotBlank() }?.let { description ->
            ...
            if (Shop.listType != ListType.Linear && foregroundFragment !is SearchFragment)
    ...
    private fun default() {
        ...
        if (adapter is ProductListAdapter && (Shop.listType == ListType.Linear || foregroundFragment is SearchFragment))
            ...
            onClick(1000) {
                if (adapter.canMealProductButtonAction == true) {
                    ...
                    when {
                        ...
                        product?.isToppings == true || product?.isBaseToppings == true -> {
                            openToppings()
                            adapter.canMealProductButtonAction = true
                        }
                        else ->
    ...
    private fun focused() {
        ...
        focusedAmount?.setAmount(
            cart[productId]?.amount ?: unitType?.type?.multiplier ?: unitTypeWidgetGrid?.type?.multiplier ?: product?.unitType?.multiplier ?: return
        )
        focusedAmount?.callback = { needToOpenPromotedProduct ->
            if (mealCallback == null) {
                addProductToCart()
    ...
    private fun done() {
        ...
        defaultButton?.apply {
            ...
            onClick {
                when {
                    product?.product?.productType == ProductType.PACK -> {
                        AddOrEditMealDialog.open { openMeal(product) }
                    }
                    product?.isToppings == true || product?.isBaseToppings == true -> openToppings()
    ...
    private fun addProductToCart(
        ...
        addToCart(toppings, options, baseToppings)?.let {
            if (dialog) {
                if (isInCart)
        ...
        ToppingsFragment.open(
            ...
            toppings?.let {
                if (mealCallback != null) {
                    ...
                    addProductToCart(it, options, baseToppings, dialog = true)
    ...
    private fun addToCart(
        ...
        return cart.set(
            ...
            unitType?.type?.type ?: focusedAmount?.unitTypeName ?:  product?.defaultUnitType ?: return null,
