...
import il.co.lapita.fragments.MenuFragment
...
import il.co.lapita.order.CreditCardFragment
import il.co.lapita.order.OrderDetailsFragment
import il.co.lapita.search.CameraSearchFragment
...
class MenuBar @JvmOverloads constructor(
    ...
    private val livePendingItemsSizeObserver = Observer<Int> { size ->
        cartBadge?.isVisible = listOf(
            SearchFragment().TAG,
            CreditCardFragment().TAG,
    ...
    override fun onFinishInflate() {
        ...
        toggleBack(
            (activity?.supportFragmentManager?.fragments?.filterIsInstance<AbstractFragment>()?.size
                ?: 0) > 1
        )
        toggleSearch(true)
    }
    ...
    fun toggleSearch(isEnabled: Boolean, isBarcode: Boolean = false) {
        when {
            isBarcode -> {
                searchButton?.isVisible = true
                searchButton?.setImageResource(R.drawable.barcode)
                searchButton?.onClick {
                    CameraSearchFragment.open { barcode ->
