...
import il.co.lapita.toppings.Pizza.rotate
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (!isRealItem) {
            when {
                product.shopProductOptions?.isNotEmpty() == true -> {
                    DialogChooseOptions.open(
                        ...
                        if (type == null) {
                            ...
                            if (product.shopBaseToppings?.isNotEmpty() == true) {
                                BaseToppingsDialog.open(product) { baseToppings ->
                                    initLists()
                                    state.options.last().add(type)
                                    state.baseToppings[index] =
                                        baseToppings?.toMutableList() ?: mutableListOf()
                                    listSelectedToppings.add(0, type)
                                    listSelectedToppings.addAll(baseToppings?.takeIf { it.isNotEmpty() }
                                        ?: listOf(
                                            ShopTopping(
                                                -1,
                                                0.0,
                                                BaseTopping("ללא שינוי", null, null)
                                            )
                                        ))
                ...
                product.shopBaseToppings?.isNotEmpty() == true -> {
                    BaseToppingsDialog.open(product) { baseToppings ->
                        initLists()
                        state.baseToppings[index] = baseToppings?.toMutableList() ?: mutableListOf()
                        listSelectedToppings.addAll(baseToppings?.takeIf { it.isNotEmpty() }
                            ?: listOf(ShopTopping(-1, 0.0, BaseTopping("ללא שינוי", null, null))))
                        view.isVisible = true
                        setDataToAdapterSelectedToppings()
                    }
                }
            ...
            product.toppings?.forEach { toppingsOnPizza[it.id] = mutableListOf() }
                ...
                if (product.isBaseToppings) {
                    state.baseToppings[index] = product.shopBaseToppings?.filter { shopTopping ->
                        cartItem?.baseToppings?.get(index)
                            ?.firstOrNull { it.first == shopTopping.id } != null
                    }?.toMutableList() ?: mutableListOf()
                    listSelectedToppings.addAll(state.baseToppings[index].takeIf { it.isNotEmpty() }
                        ?: listOf(ShopTopping(-1, 0.0, BaseTopping("ללא שינוי", null, null))))
                }
            }
            ...
            SelectedPizzaToppingsAdapter(
                ...
                onBaseToppings = {
                    BaseToppingsDialog.open(
                        shopProduct = product,
                        selectedBaseToppings = state.baseToppings[index]
                    ) { baseToppings ->
                        baseToppings?.let {
                            state.baseToppings[index] = baseToppings.toMutableList()
                            listSelectedToppings =
                                catch { state.options[index].first() }?.let { mutableListOf(it) }
                                    ?: mutableListOf()
                            listSelectedToppings.addAll(baseToppings.takeIf { it.isNotEmpty() }
                                ?: listOf(
                                    ShopTopping(
                            ...
                            listSelectedToppings.addAll(state.selected[index])
                ...
                ConfirmDialog.deleteAdditionalProduct {
                    toppingsFragment?.pizzaViewPagerAdapter?.run {
                        ...
                        if (state.selectedIds?.isNotEmpty() == true) {
                            ...
                            toppingsFragment?.onToppingsSave?.invoke(null, null, null, null)
                            activity?.navigateBack()
                        }
                    }
                }
    ...
    private fun drawTopping(slice: Int, idTopping: Int) {
        ...
        main {
            (imageSlice?.drawable as? BitmapDrawable)?.bitmap?.let { bitmap ->
                val bitmapRotated =
                    Pizza.getBitmapByCodename(product.toppings?.firstOrNull { it.id == idTopping }?.topping?.codename)
                        ?.rotate(
