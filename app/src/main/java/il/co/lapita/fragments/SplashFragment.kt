...
import il.co.lapita.utilities.preloadImage
...
class SplashFragment : AbstractFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        developer?.onClick { openWebBrowser("https://my-shop.co.il/") }
        routeUser()
    }
    ...
    private fun setShopOld() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            Remote.setShop()?.also { Database.shop = it }?.let { shop ->
                ...
                val list = (cartProducts?.mapNotNull { it.shopProductsAssociatedIds } ?: mutableListOf()).flatten().distinctBy { it }
                Locator.database.cart.getPromotedProducts(list)
