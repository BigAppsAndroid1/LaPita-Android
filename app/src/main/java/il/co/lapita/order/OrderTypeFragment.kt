...
import il.co.lapita.utilities.disable
import il.co.lapita.utilities.enable
import il.co.lapita.widgets.CustomDatePickerDialog
...
import il.co.lapita.data.Database.shop
import il.co.lapita.data.ShopBranch.Companion.BRANCH_KEY
import il.co.lapita.dialogs.MinimumForOrderAlertDialog
...
class OrderTypeFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (cart.products.values.mapNotNull { it.product?.shopProductsAssociatedIds }
                .flatten().distinctBy { it }.filter { it !in cart.products.values.map { prod-> prod.productId } }
                .mapNotNull { cart.livePromotedProducts.value?.firstOrNull { product -> product.product.id == it } }.toList().isNotEmpty())
        ...
        user?.preferredOrderType?.let { orderTypeString ->
            enumValue<OrderType>(orderTypeString)?.let { orderType ->
                when (orderType) {
                    ...
                    OrderType.Sit -> pickupCheckBox?.performClick()
                }
