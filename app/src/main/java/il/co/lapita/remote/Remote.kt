...
import il.co.lapita.BuildConfig
import il.co.lapita.R
import il.co.lapita.data.*
import il.co.lapita.data.Network
import il.co.lapita.dialogs.InfoDialog
import il.co.lapita.fragments.InfoFragment
import il.co.lapita.infrastructure.App
import il.co.lapita.infrastructure.Locator
import il.co.lapita.infrastructure.Locator.repository
import il.co.lapita.order.OrderTypeFragment
import il.co.lapita.utilities.getString
import il.co.lapita.utilities.getWeekDay
import il.co.lapita.utilities.toByteString
...
object Remote {
    ...
    private const val domain = "https://lapita807app.my-shop.co.il/api/"
    //"https://myshop-bigapps.ngrok.io/api/"
 //   "https://myshop.bigapps.co.il/api/"
    ...
    data class Response<T>(
        ...
        val promotionCoupon: JSONObject?,
    ...
    suspend fun setShop(
        ...
        val args = mutableMapOf<String, Any?>().apply {
            ...
            //v2 identifier
            put("source", "app")
            put("version", BuildConfig.VERSION_NAME+"_"+BuildConfig.VERSION_CODE)
        }
        val result = action(
            name = "set-shop",
            arguments = args,
    ...
    suspend fun makeOrder(
        ...
        when (order.paymentType) {
            PaymentType.Credit -> if (order.paymentType == PaymentType.Credit && creditTxIndex == null && creditTxConfirm == null && creditTxHolderId == null) return null
            PaymentType.Cibus -> if (database.cibus.isNullOrEmpty()) return null
            PaymentType.Tenbis -> if (database.tenbis.isNullOrEmpty()) return null
            PaymentType.Goodi -> if (database.goodi.isNullOrEmpty()) return null
            else -> {
            }
        }
        ...
        when (order.paymentType) {
            PaymentType.Cibus -> arguments["paymentCard"] = database.cibus
            PaymentType.Tenbis -> arguments["paymentCard"] = database.tenbis
            PaymentType.Goodi -> arguments["paymentCard"] = database.goodi
            else -> {
            }
        }
        ...
        return action(
            ...
            Database.promotionCoupon = this.promotionCoupon?.parsePromotionCoupon()
    ...
    //must rewrite logic for using items. See the output of the function to understand how it should be working
            ...
            meals?.isNullOrEmpty() == false -> {
                ...
                meals?.forEachIndexed { _, meal ->
                    if (meal.first) {
                        ...
                        meal.second.forEachIndexed { index, mealProduct ->
                            maxIndex += 1
                            when {
                                mealProduct.toppings?.flatten()
                                    .isNullOrEmpty() && mealProduct.baseToppings?.flatten()
                                    .isNullOrEmpty() -> {
                                    ...
                                    mealString += "&$name[$maxIndex][amount]=${mealProduct.amount ?: 1}"
                                ...
                                mealProduct.baseToppings?.flatten()
                                    ?.isNotEmpty() == true && mealProduct.toppings?.flatten()
                                    ?.isEmpty() == true -> {

                                    mealString += mealProduct.baseToppings.map { baseToppings ->
                                        var baseToppingsItem = ""
                                        maxIndex += 1
                                        baseToppingsItem += "&$name[$maxIndex][shopProductId]=${mealProduct.shopProduct.id}"
                                        baseToppingsItem += "&$name[$maxIndex][unitType]=${mealProduct.shopProduct.unitType.type}"
                                        baseToppingsItem += "&$name[$maxIndex][amount]=${mealProduct.amount ?: 1}"
                                        baseToppingsItem += "&$name[$maxIndex][relatedToPack]=${mealIndex}"
                                        baseToppingsItem += "&$name[$maxIndex][levelId]=${
                                            this@toQuery.product?.product?.levels?.get(
                                                mealProduct.level
                                            )?.id
                                        }"
                                        baseToppingsItem += if (mealProduct.shopProduct.shopProductOptions?.isNotEmpty() == true) {
                                            "&$name[$maxIndex][shopProductOptionId]=${
                                                mealProduct.options?.first()?.first()?.first
                                            }"
                                        } else ""
                                        baseToppingsItem += "&$name[$maxIndex][shopProductOptionIsPaid]=${if (mealProduct.areOptionsFree) 0 else 1}"
                                        baseToppingsItem += baseToppings.mapIndexed { i, baseT -> "$name[$maxIndex][shopBaseToppingIds][$i]=${baseT.first}" }
                                            .filter { it.isNotEmpty() }.joinToString("&")
                                            .let { "&$it" }.takeIf { it.length > 1 } ?: ""
                                        baseToppingsItem
                                    }.joinToString { "$it" }
                                }
                                else -> {
                                    ...
                                    mealProduct.toppings?.first()?.mapIndexed { i, id ->
                                        ...
                                        topping += catch {
                                            mealProduct.baseToppings?.get(i)
                                                ?.takeIf { it.isNotEmpty() }
                                                ?.mapIndexed { index, pair -> "$name[$maxIndex][shopBaseToppingIds][$index]=${pair.first}" }
                                                ?.filter { it.isNotEmpty() }?.joinToString("&")
                                                .let { "&$it" }
                                        } ?: ""
                                        ...
                                        ?.takeIf { it.length > 1 }?.let {
                                            mealProductWithToppings += it
                                        } ?: mealProduct.baseToppings?.forEach { baseToppings ->
                                            maxIndex += 1
                                            mealProductWithToppings += catch {
                                                baseToppings.takeIf { it.isNotEmpty() }
                                                    ?.mapIndexed { index, pair -> "$name[$maxIndex][shopBaseToppingIds][$index]=${pair.first}" }
                                                    ?.filter { it.isNotEmpty() }?.joinToString("&")
                                                    .let { "&$it" }
                                            }
                                    ...
                                    mealProductWithToppings += "&$name[$maxIndex][levelId]=${this@toQuery.product?.product?.levels?.get(mealProduct.level)?.id}"
                ...
                toppings?.forEachIndexed { toppingIndex, topping ->
                    maxIndex += 1
                    var productBaseToppings = catch {
                        baseToppings?.get(toppingIndex)
                            ?.takeIf { it.isNotEmpty() }
                            ?.mapIndexed { index, pair -> "$name[$maxIndex][shopBaseToppingIds][$index]=${pair.first}" }
                            ?.filter { it.isNotEmpty() }?.joinToString("&").let { "&$it" }

                    } ?: ""
                    if (productBaseToppings.lastOrNull() != '&') productBaseToppings += "&"
                    itemsString += productBaseToppings
                    ...
                    itemsString += generateProductBaseFields(
                        product!!,
                        options,
                        toppingIndex,
                        name,
                ...
                } ?: baseToppings?.forEachIndexed { baseToppingIndex, _ ->
                    maxIndex += 1

                    itemsString += catch {
                        product?.shopBaseToppings
                            ?.mapIndexed { i, topping ->
                                if (topping.id !in (baseToppings?.get(baseToppingIndex)
                                        ?.map { it.first } ?: mutableListOf())
                                ) "" else "$name[$maxIndex][shopBaseToppingIds][$i]=${topping.id}"
                            }
                            ?.filter { it.isNotEmpty() }?.joinToString("&").let { "&$it" }
                    } ?: ""
    ...
    private fun generateProductBaseFields(
        product: ShopProduct,
        ...
        var res = "$name[$maxIndex][shopProductId]=${product.id}"
        res += "&$name[$maxIndex][unitType]=$unitTypeName"
        res += "&$name[$maxIndex][amount]=$amount"
        res += "&$name[$maxIndex][comment]=${comment ?: ""}"
        res += catch {
            product.shopProductOptions
                ?.mapIndexed { i, topping ->
                    if (topping.id !in (options?.get(index)?.map { it.first }
                            ?: mutableListOf())
                    ) "" else "$name[$maxIndex][shopProductOptionId][$i]=${topping.id}"
                }
                ?.filter { it.isNotEmpty() }?.joinToString("&").let { "&$it" }
        } ?: ""
        return res
    ...
    fun JSONObject.parseShopJson(): Shop = let { data ->
        getJSONObject("shop").run {
            Shop(
                ...
                associatedDescription = data.string("associatedDescription"),
                about = string("about"),
                ...
                welcomeMessage = catch(true) { string("splash")?.takeIf { it.isNotBlank() } }
            )
        }
    }
    ...
    private fun JSONObject.parseProduct() =
        ShopProduct(
            ...
            shopBaseToppings = getList<JSONObject, ShopTopping>("shopBaseToppings") { parseShopTopping() },
            shopProductOptions = catch(true) { getList<JSONObject, ShopTopping>("shopProductOptions") { parseShopTopping() } },
            shopProductsAssociatedIds = catch(true) {
                val list = mutableListOf<Int>()
                val jsonArray = getJSONArray("shopProductsAssociatedIds")
    ...
    private fun JSONObject.parseBaseProduct() =
        BaseProduct(
            ...
            description = string("description"),
            toppingsDescription = catch(true) { getString("toppingsDescription").takeIf { it != "null" } },
            baseToppingsDescription = catch(true) { getString("baseToppingsDescription").takeIf { it != "null" } },
            toppings = getList<JSONObject, BaseTopping>("toppings") { parseBaseTopping() },
            optionsDescription = catch(true) { getString("optionsDescription").takeIf { it != "null" } },
            levels = catch(true) { getList<JSONObject, Level>("levels") { parseLevel() } },
    ...
    private fun JSONObject.parsePromotionCoupon(): PromotionCoupon = PromotionCoupon(
        code = getString("code"),
        discount = getInt("discount"),
        dateToFormat = getString("dateToFormat")
    )
    ...
    private fun JSONObject.parseShopTopping() =
        ShopTopping(
            id = getInt("id"),
            price = catch(true) { getDouble("price") } ?: 0.0,
            topping = catch(true) { getJSONObject("topping").parseBaseTopping() }
                ?: catch(true) { getJSONObject("productOption").parseBaseTopping() }
                ?: getJSONObject("baseTopping").parseBaseTopping()
    ...
    private fun JSONObject.parseBaseTopping() =
        BaseTopping(
            name = getString("name"),
            codename = catch(true) { getString("codename") },
            imageKiosk = catch(true) { string("imageKiosk") }
        )
