...
import il.co.lapita.remote.ZCredit
...
object Database {
    ...
    var isNotShownForSession: Boolean = false
    ...
    private val KEY_NOTIFICATIONS get() = "notifications_${shop?.id}"
    ...
    var goodi: String? = null
        set(value) {
            field = value
            if (value == null)
                sharedPreferences.delete(PaymentType.Goodi.name)
            else
                save(value, PaymentType.Goodi.name)
        }
        get() = get(PaymentType.Goodi.name)

    var cibus: String? = null
        set(value) {
            field = value
            if (value == null)
                sharedPreferences.delete(PaymentType.Cibus.name)
            else
                save(value, PaymentType.Cibus.name)
        }
        get() = get(PaymentType.Cibus.name)

    var tenbis: String? = null
        set(value) {
            field = value
            if (value == null)
                sharedPreferences.delete(PaymentType.Tenbis.name)
            else
                save(value, PaymentType.Tenbis.name)
        }
        get() = get(PaymentType.Tenbis.name)
    ...
    var promotionCoupon: PromotionCoupon? = null
